...
module Greeting_Client(hello) where
...
send_hello op = do
  ...
  T.writeMessageBegin op ("hello", T.M_CALL, seqn)
  write_Hello_args op (Hello_args{})
...
recv_hello ip = do
  ...
  res <- read_Hello_result ip
  T.readMessageEnd ip
  P.return $ hello_result_success res
